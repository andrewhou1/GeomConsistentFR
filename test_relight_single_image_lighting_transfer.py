...
def main():
    ...
    with torch.no_grad():
        ...
        name_parts = curr_img_name.split('.')
        ...
        cv2.imwrite('lighting_transfer_result/'+name_parts[0]+'_shadow_mask.png', 255.0*shadow_mask_weights[0, :, :].cpu().numpy()*np.reshape(curr_mask_fill_nose.numpy(), (model.img_height, model.img_width)))
        cv2.imwrite('lighting_transfer_result/'+name_parts[0]+'_albedo.png', 255.0*albedo[0, :, :, ::-1]*curr_mask_fill_nose_3_channels)
        cv2.imwrite('lighting_transfer_result/'+name_parts[0]+'_depth.png', 255.0*depth[0, :, :, :]*curr_mask_fill_nose.numpy())
        cv2.imwrite('lighting_transfer_result/'+name_parts[0]+'_shading.png', 255.0*final_shading[0, :, :]*np.reshape(curr_mask_fill_nose.numpy(), (model.img_height, model.img_width)))
        cv2.imwrite('lighting_transfer_result/'+name_parts[0]+'_surface_normals.png', surface_normals[0, :, :, ::-1]*curr_mask_fill_nose_3_channels)
